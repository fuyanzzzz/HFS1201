'''
-------------------------------------------------
File Name: test_discard_weight.py
Author: LRS
Create Time: 2023/5/4 20:57
-------------------------------------------------
'''
'''
-------------------------------------------------
    ...
    def get_state(self):
        ...
        for index, job in enumerate(wait_for_allocation):
            if self.stage_index == stages_num-1:
                ...
                n_state[index] = (ddl_ten[index] - remanin_process_time[index])
            else:
                n_state[index] = (self.job_execute_time[(self.stage_index+1,job)][0] - remanin_process_time[index])/ddl_weight[job]
                n_state[index] = (self.job_execute_time[(self.stage_index+1,job)][0] - remanin_process_time[index])
    ...
    def get_reward(self):
        ...
        for job in sorted_jobs:
            ...
            if self.job_execute_time[(stages_num-1,int(job))][1] < ect_windows[int(job)]:
                ...
                # penalty_value += (ect_windows[job] - self.job_execute_time[(stages_num-1,int(job))][1])
            if self.job_execute_time[(stages_num-1,int(job))][1] > ddl_windows[job]:
                penalty_value += (self.job_execute_time[(stages_num-1,int(job))][1] - ddl_windows[job]) * ddl_weight[job]
                # penalty_value += (self.job_execute_time[(stages_num-1,int(job))][1] - ddl_windows[job])
            else:
                penalty_value += 0
            ...
            if done:
                ...
                if memory.size() > BEGIN_LEARN_SIZE:
                    ...
                    if epoch % 100 == 0:
                        ...
                        plt.draw()
                        plt.savefig('./img0511/pic-{}.png'.format(int(epoch/100)))
                        plt.pause(1)
