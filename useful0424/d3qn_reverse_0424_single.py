...
def job_assignment(stage,job,machine,schedule,job_completion_time,machine_completion_time):
    ...
    if stage == 0:
        ...
        if pro_job == -1:
            ...
            ect_value = machine_completion_time[(stage,machine)][1]
    ...
    # 如果非第一阶段
        ...
        if pro_job == -1:
            ...
            # ect = max（该机器上最后一个工件的完工时间a，该工件在上一个阶段上的完工时间b） + 切换时间c  【这里a,b ！＝ 0】
            job_on_pro_machine = job_completion_time[stage-1][job]
            ect_value = max(job_on_pro_machine,machine_completion_time[(stage,machine)][1])  # 机器最早可开始加工时间 = 上个工件的完工时间 + 切换时间
...
for epoch in count():
    ...
    while True:
        ...
        # 根据（状态，动作）得到step序列
        next_state, reward, done,obj,schedule= env.step(action)
