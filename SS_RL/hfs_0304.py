...
def inintal_solution():
    ...
    improve_obj,all_job_block,_ = idle_time_insertion(schedule)
    ...
    improve_lsl_obj,all_job_block,_ = idle_time_insertion(schedule_lsl)
    ...
    improve_osl_obj, all_job_block,_ = idle_time_insertion(schedule_osl)
...
def destruction_reconstruction(job_sort_inital,d_list):
    ...
    for r_job in remove_jobs:
        ...
        while pos <= len(job_sort_inital):
            ...
            cur_obj, all_job_block,_ = idle_time_insertion(cur_schedule)      # 每进行一次重构，也需要进行空闲插入程序
            if cur_obj < best_obj:
                best_obj = cur_obj
    ...
    for machine in range(machine_num_on_stage[-1]):
        ...
        while job_num_machine > 0:
            ...
            if len(all_job_block) != 0:        # 如果当前工件块右侧存在工件
                ...
                job_completion_time_1 = job_makespan[job_before_idle]  # 当前工件块最后一个工件的完工时间
                ...
                idle_2 = later_block_start_time - job_completion_time_1
            else:
            ...
            if early_job_weight > delay_job_weight:
                ...
                job_completion_time[stage] = job_makespan
            else:
                job_num_machine -= 1    # 而且对于工件块有合并的选项

        all_job_block.insert(0,job_block)

    return obj,all_job_block,job_completion_time
...
if __name__ == '__main__':
    ...
    for index, file_name in enumerate(txt_files):
        ...
        for i in range(10):
            ...
            improve_obj, all_job_block,job_completion_time = idle_time_insertion(schedule)
