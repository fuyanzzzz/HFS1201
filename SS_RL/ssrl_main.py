...
for index, file_name in enumerate(txt_files):
    ...
    print('{0}   {1}   {2}   {3}   {4} '.format(index,file_name,round(duration,2),rl_.config.ture_opt,rl_.best_opt), file=fp)
    fp.close()
    if index % 20 == 0:
        ...
        plt.tight_layout()
        # plt.pause(0.1)  # 用于动态展示图像
        plt.savefig('./img0.05_0.8/pic-{}.png'.format(int(index)))
        fp = open('./0.05_0.8.txt', 'a+')
        print(index,rl_.q_table, file=fp)
        fp.close()
