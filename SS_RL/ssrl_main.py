    ...
    for i_tiankou in range(9,10):
        ...
        for i_iter in range(4):
            for index, file_name in enumerate(txt_files):
                ...
                rl_ = RL_Q(N_STATES,ACTIONS,hfs.inital_refset,q_table,file_name,len(INDEX),i_stop_iter,a,i_trial,lr,jingying,discount_rate)
    ...
    for index, file_name in enumerate(txt_files):
        for tiankou_index in range(9,10):
            ...
            for i in range(20):
                ...
                rl_ = RL_Q(N_STATES, ACTIONS, hfs.inital_refset, q_table, file_name, i_iter_file_name,i_stop_iter, a, i_trial, lr, jingying, discount_rate)      # ！！！！！！倒数第二个参数修改，对应的动作不应该再有随机了！！！！！！！！！
                best_opt_execute, CUM_REWARD_case = rl_.rl_execute()
                best_opt_list.append(best_opt_execute)
                with open('./tiankou0304_termination/real_date_MDP_real_para_1.txt', 'a+') as fp:
            ...
            with open('./tiankou0304_termination/real_data_result_para_1.txt', 'a+') as fp:
                print(file_name, rl_.config.ture_opt, round(sum(opt_list) / len(opt_list), 2),
