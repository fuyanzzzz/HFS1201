...
x = MODEL.addVars(stage_num, machine_num, job_num+1, job_num, vtype=gurobipy.GRB.BINARY, name="")
    # 阶段l工件i的完工时间  ！！是否需要非负
...
# 创建目标函数
expression = sum(e[j] * ddl_weight[j] + t[j] * ect_weight[j] for j in range(job_num))
...
for l in range(stage_num):
    for j in range(job_num):
        MODEL.addConstr(x.sum[l,"*","*",j] == 1)

# 约束条件3
for l in range(stage_num):
    for i in range(job_num+1):
        MODEL.addConstr(x.sum[l,"*",i,"*"] <= 1)

# 约束条件4
for i in range(job_num+1):
    for j in range(job_num):
        # 约束条件
        MODEL.addConstr(c[1,i] + x.sum(1,"*",i ,j) *job_process_time[1,j] + F * (x.sum(1,"*",i ,j) - 1)<= c[1,j])

# 约束条件5
for i in range(job_num+1):
    for j in range(job_num):
        # 约束条件
        MODEL.addConstr(max(c[2,i],c[1,j]) + x.sum(2,"*",i ,j) *job_process_time[2,j] + F * (x.sum(2,"*",i ,j) - 1)<= c[2,j])

# 约束条件6
for l in range(stage_num):
    MODEL.addConstr(c[l,job_num]==0)

# 约束条件7
for l in range(stage_num):
    for j in range(job_num):
        MODEL.addConstr(c[l,j]>=0)

for j in range(job_num):
    MODEL.addConstr(e[j] >= max((ect_windows[j] - c[stage_num, j]),0))  # 约束条件8
    MODEL.addConstr(t[j] >= max((c[stage_num, j] - ddl_windows[j]),0))  # 约束条件9
